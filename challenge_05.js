...
users.split(';').forEach(user => {
    let isValid = true
    let info = user.split(',')
    if (info[0] === '' || info[1] === '' || info[2] === '') { isValid = false }
    if (info[3] !== '' && !isNumber.test(info[3])) { isValid = false }
    if (info[4] !== '' && !isText.test(info[4])) { isValid = false }
    isValid = isValid ? isAlphaNum.test(info[0]) : isValid
    isValid = isValid ? isAlphaNum.test(info[1]) : isValid
    isValid = isValid ? isEmail.test(info[2]) : isValid

    if (!isValid) {
        invalidUsers.push(user)
        secret = secret + info[1].charAt(0)
    }
})
